...
export function tweakInspectorForFloorplanner(inspector, myFloorplan, editorHelper) {
    inspector.properties = {
        ...
        height: {
            show: function (part) {
                return part.category === '' || part.category === 'MultiPurposeNode';
            },
        },
        width: {
            show: function (part) {
        ...
        thickness: {
            show: function (part) {
                return part.category === 'WallGroup' && !part.data.isDivider;
            },
        },
    ...
    Inspector.prototype.updateAllHTML = function () {
        ...
        const isPart = this.inspectedObject instanceof window.go.Part;
        if (this.inspectedObject instanceof window.go.Node) {
            ...
            for (const name in inspectedProps) {
                ...
                if (type === '') {
                    ...
                    if (typeof oldval === 'boolean')
                        ...
                        type = 'number';
                    else if (oldval instanceof window.go.Point)
                        type = 'point';
                    else if (oldval instanceof window.go.Size)
                        type = 'size';
                    else if (oldval instanceof window.go.Rect)
                        type = 'rect';
                    else if (oldval instanceof window.go.Spot)
                        type = 'spot';
                    else if (oldval instanceof window.go.Margin)
                        type = 'margin';
                }
                ...
                switch (type) {
                    ...
                    case 'arrayofnumber':
                        ...
                        break;
                    case 'point':
                        value = window.go.Point.parse(value);
                        break;
                    case 'size':
                        value = window.go.Size.parse(value);
                        break;
                    case 'rect':
                        value = window.go.Rect.parse(value);
                        break;
                    case 'spot':
                        value = window.go.Spot.parse(value);
                        break;
                    case 'margin':
                        value = window.go.Margin.parse(value);
                        break;
                    case 'checkbox':
                        value = input.checked;
                        break;
                ...
                if ((input && name === 'height') ||
                    ...
                    if (isNaN(parseFloat(value)) || parseFloat(value) <= 0) {
                        const oldVal = this.inspectedObject.data[name];
                        value = oldVal;
                    }
                }
                ...
                if (this.inspectedObject.category === 'WallGroup') {
                    const wall = this.inspectedObject;
                    const wrt = myFloorplan.toolManager.mouseDownTools.elt(3);
                    wrt.performMiteringOnWall(wall);
                    const set = new window.go.Set();
                    set.add(wall);
